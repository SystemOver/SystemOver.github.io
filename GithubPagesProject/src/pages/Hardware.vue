...
const StreamingHardwareData = ref([
...
const copyList = async () => {
  var text = 'Raw List of Hardware: \n'
  StreamingHardwareData.value.forEach((x) => (text += '- ' + x.label + '\n'))
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Copied List',
      detail: 'copied entrie list of Hardware that i use to your clipboard',
      life: 10000,
    })
  ...
  <div class="grid grid-cols-1 sm:grid-cols-5 mx-5 sm:mx-0">
    ...
    <div class="col-start-1 col-span-1 sm:col-start-2 sm:col-span-3">
      <div class="justify-self-center justify-items-center">
        <p>What kind of</p>
        <h1 class="text-6xl Title text-shadow-lg text-shadow-green-500 mb-2 mt-2 hue-spin-hover">
          Hardware
        </h1>
        <p>do i use?</p>
      </div>
      ...
      <div>
        <p>
          ...
          Hardware that i use, so i will put it <a @click="copyList()">here</a>, but under that you
          will find a more detailed description and explaination why, how, when i use specific
          Hardware. Anyways here is the list:
        </p>
        <div class="grid grid-cols-2 mt-8">
          <div class="col-span-1">
            ...
            <Listbox
              ...
              :options="
                StreamingHardwareData.filter((x) => x.type === 'Streaming').map((row) => row.label)
              "
              filter
              checkmark
              multiple
          ...
          <div class="col-span-1 col-start-2">
            ...
            <Listbox
              ...
              :options="
                StreamingHardwareData.filter((x) => x.type === 'NonStreaming').map(
                  (row) => row.label,
                )
              "
              filter
        ...
        <div v-if="selectedStreamingIndexes.length + selectedNonStreamingIndexes.length === 0">
          <h3 class="text-4xl justify-self-center p-10 text-gray-500">
            Select a Hardware to learn more About it
          </h3>
        </div>

        <div
          v-for="value in StreamingHardwareData.filter(
            (x) =>
